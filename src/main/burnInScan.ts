...
import type { BurnInFinding, PreviewFrame, Stack } from '@shared/types'
...
async function scanStack(stack: Stack): Promise<BurnInFinding | null> {
  ...
  if (first.width !== second?.width || first.height !== second?.height) {
    const { regions } = findOverlayRegions(luminance(first, stack), null, first.width, first.height, stack.masks)
